package com.xmm.cactus.imageloader;
...
import com.xmm.cactus.util.NetWorkUtils;
...
public class GlideImageLoaderProvider extends BaseImageLoaderProvider {
    ...
    public void loadImage(Context ctx, ImageLoader img) {
        ...
        boolean flag= NetWorkUtils.getOnlyWifiLoadImg(ctx);
        //如果不是在wifi下加载图片，直接加载
        if(!flag){
            loadNormal(ctx,img);
            return;
        ...
        if(strategy == ImageLoaderUtil.LOAD_STRATEGY_ONLY_WIFI){
            ...
            if(netType == NetWorkUtils.NETWORKTYPE_WIFI) {
                loadNormal(ctx, img);
            } else {
